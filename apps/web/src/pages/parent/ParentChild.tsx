    ...
    <div className="flex flex-col gap-4 py-4">
      ...
      {level_ranking && (
        <div
          className="rounded-2xl p-4"
          style={{
            background: 'linear-gradient(135deg,rgba(129,140,248,0.15),rgba(59,130,246,0.08))',
      ...
      {level && level.level && (
        <div
          className="rounded-2xl p-4"
          style={{
            background: `linear-gradient(135deg,${LEVEL_COLOR[level.level] ?? '#6366f1'}22,${LEVEL_COLOR[level.level] ?? '#6366f1'}08)`,
            border: `1px solid ${LEVEL_COLOR[level.level] ?? '#6366f1'}33`,
          }}
        >
          <p
            className="mb-3 text-xs font-semibold uppercase tracking-wide"
            style={{ color: `${LEVEL_COLOR[level.level] ?? '#6366f1'}cc` }}
          >
          ...
          <div className="flex items-center gap-3">
            <div
              ...
              style={{
                background: `${LEVEL_COLOR[level.level] ?? '#6366f1'}22`,
                color: LEVEL_COLOR[level.level] ?? '#6366f1',
              }}
            >
              {level.level}
            </div>
