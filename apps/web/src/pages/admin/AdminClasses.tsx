            ...
            <div className="mb-3 flex flex-wrap gap-2">
              {languages?.map((l) => (
                <button
                  ...
                  style={{
                    background: langId === l.id ? (l.color ?? '#6366f1') : 'var(--surface-2)',
        ...
        <div className="mb-3 flex gap-2">
          {LEVELS.map((l) => (
            <button
              ...
              className="press flex-1 rounded-xl py-2 text-xs font-bold"
              style={{
                background: level === l ? (LEVEL_COLOR[l] ?? '#6366f1') : 'var(--surface-2)',
...
> = {
  ...
  EXAM: { status: 'COMPLETED', label: 'Завершить', color: '#818cf8' },
};
